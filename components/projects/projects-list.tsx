...
const statusClass: Record<ProjectStatus, string> = {
  ...
  Processing: "bg-amber-300/10 text-amber-100 border-amber-300/20",
      ...
      {projects.map((project) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-sm font-medium leading-5 text-stone-100">
              {project.title}
            </h3>
            <span
          ...
          <div className="mt-3 flex items-center justify-between gap-3 text-xs text-stone-500">
            <span>{project.duration ?? project.activity}</span>
