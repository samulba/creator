  ...
  return (
    <div className="space-y-5">
      <section className="rounded-xl border border-white/8 bg-white/[0.025] p-5">
        <p className="text-xs text-stone-500">Selected story direction</p>
        <h3 className="mt-2 text-xl font-semibold">
          A Ghost Face sacrifices the entire match just to chase the player.
        </h3>
        <p className="mt-3 text-sm text-stone-400">
          This gives the video a simple premise viewers can understand immediately.
        </p>
        <button className="mt-4 rounded-md border border-white/10 px-3 py-2 text-sm hover:bg-white/5">
      ...
      <section className="grid gap-3 md:grid-cols-2">
        {alternatives.map((angle) => (
          <button
            key={angle}
            className="rounded-lg border border-white/8 bg-white/[0.025] p-4 text-left text-sm text-stone-300 hover:bg-white/[0.05]"
          >
            {angle}
          </button>
        ))}
